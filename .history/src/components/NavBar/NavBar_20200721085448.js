...
export default function NavBar({ navlinks, vizSensor }) {
    ...
    useEffect(() => {
        if(vizSensor.header) {setCurrentLink(''); history.location.hash='';}
        else{ 
            if(vizSensor.mappa) {setCurrentLink('mappa'); history.location.hash='mappa';}
            else{
                if(vizSensor.referente) {setCurrentLink('referente'); history.location.hash='referente';}
                else{
                    if(vizSensor.viaggio) {setCurrentLink('viaggio'); history.location.hash='viaggio';}
                    else{
                        if(vizSensor.info) {setCurrentLink('info'); history.location.hash='info';}
                    }
                }
            } 
        }
