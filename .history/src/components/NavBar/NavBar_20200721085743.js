...
export default function NavBar({ navlinks, vizSensor }) {
    ...
    useEffect(() => {
        if(vizSensor.header) {setCurrentLink(''); history.replace('');}
        else{ 
            if(vizSensor.mappa) {setCurrentLink('mappa'); history.replace('#mappa');}
            else{
                if(vizSensor.referente) {setCurrentLink('referente'); history.replace('referente');}
                else{
                    if(vizSensor.viaggio) {setCurrentLink('viaggio'); history.replace('viaggio');}
                    else{
                        if(vizSensor.info) {setCurrentLink('info'); history.replace('info');}
                    }
                }
            } 
        }
